...
제임스 휘태커의 10분 테스트 계획 (생산성과 관련된 프로세스로 참고하기 좋다)
 ...
 - TE 면접에 대한 내용
  1. 제품에 호기심이 많고, 흥미를 가진 지원자, 강인한 성격
  2. a 카운트 웹페이지 질문
  3. 명세에 의문을 가질 줄 알아야 한다.
  4. 호기심 많고 열정적인 엔지니어를 원함
  5. 지속적 발전, 배울점이 있는 후보자를 원한다.
  6. "애매한 상황들을 얼마나 잘 다루는지가 중요하다."
  
 - 메뉴얼 테스트 제거에 관한 가이드라인
  1. 항상 통과하는 낮은 priority test 때문에 높은 priority test 수행에 영향을 준다면 해당 테스트 제거 (한정된 자원을 효율적으로 사용하기 위해)
  2. 거짓양성(False Positive) or 신뢰할 수 없는 자동화 테스트에서 불필요한 부분은 제거
  
 - 유지 관리 모드 전 가이드라인
  1. 어려운 문제점은 중단하지 말고 바로 잡아라
  2. 테스트 프로그램에 대한 실용적인 문서를 작성하라
  3. 문제 발생시, 단계적으로 확대되는 계획을 작성 후 따라라
  4. 항상 일해왔던 프로젝트에 투임되거나 도와줄 준비를 해라
  
 - 퀄리티 봇
  1. 구글에서는 봇을 통해 상당 부분을 수동 -> 자동 테스트로 대체할 수 있었다. but, 탐색적 테스팅, 위험 분석, 사용자에 대한 고려는 여전히 수행해야 한다.
 
 - BITE 실험(Browser Integrated Test Environment) 'BITE' 는 아래와 같은 항목들을 자동으로 추가해 준다.
  1. 스크린 샷
  2. 마킹된 부분의 HTML 엘리먼트
  3. 재현 스텝이 백그라운드에서 자바스크립트로 저장
  4. 맵의 특정 디버그 URL
  5. 모든 브라우저와 OS 정보
  
 - BITE를 이용한 수동 테스트와 탐색적 테스트
  1. 테스트 자동 할당(버튼 클릭으로 동작)
  2. 각 테스트는 URL 보유
  3. 자동 URL 이동, 테스트 절차 재현, 테스트 페이지에서 수행될 검증 보여줌
  4. PASS 클릭 시, BITE버그 리포팅 UX 띄움
  5. 크라우드 소싱 테스트에 효과적
 
 - BITE의 레이어
  1. 임의의 스크립트를 실행할 수 있고, 그것을 테스트중인 페이지에 주입할 수 있다.
  
 구글 테스트 분석
 - 스프레드시트 접근법 단점
  1. 공통된 스키마 공유 x, 데이터를 일괄적으로 보여줄 방법 x
  2. ACC에서 배운 네이밍 방법은, 스프레드시트에서는 적절 x
  3. 중앙저장소가 없고 필요할 때만 공유되서 가시성 제한
  4. 위험 분석을 제품지표에 연결하기 위한 방법과 스크립트 작성이 어려움
 
 - GTA(Google Test Analytics)를 통해 프로젝트에 위험도 계산을 쉽게 연결할 수 있다.
 
 - 테스터들이 컴포넌트 테스팅에 집중하는 것이 아니라 제품 간의 속성 테스트에 집중하게 했을 때 효율이 상승
 
 - 테스팅 업무 흐름
  1. GTA를 통한 테스트 계획 : 위험 기반의 신속하고 자동화된 업데이트
  2. 테스트 커버리지 : 크롤링 봇을 통한 변경 감지 후 담당자에게 전달
  3. 버그 평가 : 차이점을 현재 존재하는 버그와 테스트들과 묶어서 전달
  4. 탐색적 테스팅 : 크라우드 소싱 테스터와 얼리 어댑터들을 지속적으로 탐색적 테스팅 수행
  5. 버그 보고 : 굉장히 편한 UI(마우스클리)으로 버그 보고 가능
  6. 선별과 디버깅 : 실시간 상황판, 오류 데이터를 포함한 버그, 원클릭 재현 방법등을 개발자나 테스트매니저에게 보고
  7. 새로운 버전 배포 후 다시 1단계로

린제이 웹스터(구글 문서도구의 TE) 인터뷰 내용
- 새로운 프로젝트에 대해 사용자 관점에서 제품을 알아본다. 개인적으로 계정 및 데이터를 생성해서 이용해 본다.
- 제품을 끝에서 끝까지(end to end) 이해 (설계문서, 주요 기능 설계 문서 등)
- 프로젝트의 상태를 살핌 (버그개수, 이슈들의 그룹화 여부, 시간순으로 버그 확인, 발견-수정된 버그 비율도 확인)
- Code repository도 확인(클래스, 단위 테스트 등)
- 자동화 테스트 검토
- 팀원들이 어떻게 의사소통을 하고, 테스터에게 기대하는 것을 어떻게 질문하는지, 그리고 그 팀과 같이 소통한다.
- 앱을 기능에 따라 의미 있는 컴포넌트로 분리
- 테스트 우선순위 결정(앱에 기능과 가용성 위주로)
- 버그 저장소 살피기 (같은 종류의 컴포넌트를 모으고, 중복관리)
- 컴포넌트의 사용자 스토리를 만듦, 상세한 기능에는 TC를 작성하고, 컴포넌트에 대해서는 더 큰 사용자 스토리와 링크를 공유
- 보안성, 호환성, 통합성, 탐색적 테스팅, 회귀 테스팅, 성능 테스팅, 부하 테스팅을 위주로 다룸
- 위 사항들을 최신으로 유지하고 SET와 주로 구성이나 피드백에 대해 
